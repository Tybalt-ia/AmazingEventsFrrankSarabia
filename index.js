...
for (let i = 0; i < data.events.length; i++) {
  if (!checkbox.includes(data.events[i].category)) {
    checkbox.push(data.events[i].category)
  }
...
function pintarCheckbox(events) {
  for (let i = 0; i < events.length; i++) {
    ...
    check.innerHTML = `
      <input class="form-check-input" type="checkbox" value="${events[i]}" id="flexCheck1">
